...
def train_syndiff(args):
    ...
    if args.use_ema:
        ...
        optimizer_gen_diffusive_2 = EMA(
            optimizer_gen_diffusive_2, ema_decay=args.ema_decay
        )
        optimizer_gen_non_diffusive_1to2 = EMA(
            optimizer_gen_non_diffusive_1to2, ema_decay=args.ema_decay
        )
        optimizer_gen_non_diffusive_2to1 = EMA(
            optimizer_gen_non_diffusive_2to1, ema_decay=args.ema_decay
        )
    ...
    if args.resume:
        ...
        gen_non_diffusive_1to2.load_state_dict(
            checkpoint["gen_non_diffusive_1to2_dict"]
        )
        gen_non_diffusive_2to1.load_state_dict(
            checkpoint["gen_non_diffusive_2to1_dict"]
        )
        # load G
        ...
        optimizer_gen_diffusive_2.load_state_dict(
            checkpoint["optimizer_gen_diffusive_2"]
        )
        scheduler_gen_diffusive_2.load_state_dict(
            checkpoint["scheduler_gen_diffusive_2"]
        )
        optimizer_gen_non_diffusive_1to2.load_state_dict(
            checkpoint["optimizer_gen_non_diffusive_1to2"]
        )
        scheduler_gen_non_diffusive_1to2.load_state_dict(
            checkpoint["scheduler_gen_non_diffusive_1to2"]
        )
        optimizer_gen_non_diffusive_2to1.load_state_dict(
            checkpoint["optimizer_gen_non_diffusive_2to1"]
        )
        scheduler_gen_non_diffusive_2to1.load_state_dict(
            checkpoint["scheduler_gen_non_diffusive_2to1"]
        )
        # load D
        ...
        optimizer_disc_diffusive_2.load_state_dict(
            checkpoint["optimizer_disc_diffusive_2"]
        )
        scheduler_disc_diffusive_2.load_state_dict(
            checkpoint["scheduler_disc_diffusive_2"]
        )
        # load D_for cycle
        disc_non_diffusive_cycle1.load_state_dict(
            checkpoint["disc_non_diffusive_cycle1_dict"]
        )
        optimizer_disc_non_diffusive_cycle1.load_state_dict(
            checkpoint["optimizer_disc_non_diffusive_cycle1"]
        )
        scheduler_disc_non_diffusive_cycle1.load_state_dict(
            checkpoint["scheduler_disc_non_diffusive_cycle1"]
        )

        disc_non_diffusive_cycle2.load_state_dict(
            checkpoint["disc_non_diffusive_cycle2_dict"]
        )
        optimizer_disc_non_diffusive_cycle2.load_state_dict(
            checkpoint["optimizer_disc_non_diffusive_cycle2"]
        )
        scheduler_disc_non_diffusive_cycle2.load_state_dict(
            checkpoint["scheduler_disc_non_diffusive_cycle2"]
        )
    ...
    for epoch in range(init_epoch, args.num_epoch + 1):
        for iteration, batch in enumerate(tqdm(train_loader)):
            ...
            t2 = torch.randint(
                0, args.num_timesteps, (real_data2.size(0),), device=device
            )
            ...
            if args.lazy_reg is None:
                ...
                grad_penalty = (
                    args.r1_gamma / 2 * grad1_penalty
                    + args.r1_gamma / 2 * grad2_penalty
                )
            ...
            x2_pos_sample = sample_posterior(
                pos_coeff, x2_0_predict_diff[:, [0], :], x2_tp1, t2
            )
            ...
            errG = (
                args.lambda_l1_loss * errG_cycle
                + errG_adv
                + errG_cycle_adv
        ...
        if rank == 0:
            if epoch % 10 == 0:
                ...
                torchvision.utils.save_image(
                    x2_pos_sample,
                    os.path.join(exp_path, "xpos2_epoch_{}.png".format(epoch)),
                    normalize=True,
                )
            ...
            pred1 = torch.cat(
                (
                    real_data2,
                    pred1,
                    gen_non_diffusive_1to2(pred1),
                    fake_sample2_tilda[:, [0], :],
                ),
                axis=-1,
            )
            torchvision.utils.save_image(
                pred1,
                os.path.join(exp_path, "sample1_translated_epoch_{}.png".format(epoch)),
                normalize=True,
            )

            x2_t = torch.cat((torch.randn_like(real_data2), real_data1), axis=1)
            fake_sample2 = sample_from_model(
                pos_coeff, gen_diffusive_2, args.num_timesteps, x2_t, T, args
            )
            ...
            pred2 = torch.cat(
                (
                    real_data1,
                    pred2,
                    gen_non_diffusive_2to1(pred2),
                    fake_sample1_tilda[:, [0], :],
                ),
                axis=-1,
            )
            torchvision.utils.save_image(
                pred2,
                os.path.join(exp_path, "sample2_translated_epoch_{}.png".format(epoch)),
                normalize=True,
            )
            ...
            if epoch % args.save_ckpt_every == 0:
                if args.use_ema:
                    ...
                    optimizer_gen_diffusive_2.swap_parameters_with_ema(
                        store_params_in_ema=True
                    )
                    optimizer_gen_non_diffusive_1to2.swap_parameters_with_ema(
                        store_params_in_ema=True
                    )
                    optimizer_gen_non_diffusive_2to1.swap_parameters_with_ema(
                        store_params_in_ema=True
                    )
                torch.save(
                ...
                torch.save(
                    gen_diffusive_2.state_dict(),
                    os.path.join(exp_path, "gen_diffusive_2_{}.pth".format(epoch)),
                )
                torch.save(
                    gen_non_diffusive_1to2.state_dict(),
                    os.path.join(
                        exp_path, "gen_non_diffusive_1to2_{}.pth".format(epoch)
                    ),
                )
                torch.save(
                    gen_non_diffusive_2to1.state_dict(),
                    os.path.join(
